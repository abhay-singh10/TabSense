...
chrome.runtime.onInstalled.addListener(() => {
  chrome.storage.local.get(["tabData"], function (result) {
    if (!result.tabData) {
      ...
      console.log("TabSense : Storage initialized");
    }
  });
});
...
chrome.tabs.onCreated.addListener((tab) => {
  console.log("TabSense : onCreated event triggered. Tab details:", JSON.stringify(tab));
  mostRecentTab = tab;
  
  // Check if this tab was created by our extension (like the dashboard or popup)
  const newTabUrl = tab.pendingUrl || tab.url;
  console.log(`TabSense : onCreated - Tab ID: ${tab.id}, NewTabURL: ${newTabUrl}, Status: ${tab.status}`);
  
  // If it's an extension page, don't show the popup
  if (isExtensionUrl(newTabUrl)) {
    console.log("TabSense : Not showing popup for extension page:", newTabUrl);
    return;
  }
  
  // If we already prompted for this tab, don't prompt again
  if (promptedTabs.has(tab.id)) {
    console.log("TabSense : Already prompted for tab", tab.id);
    return;
  }
  ...
  setTimeout(() => {
    ...
    chrome.tabs.get(tab.id, (currentTab) => {
      if (chrome.runtime.lastError) {
        console.log("TabSense : Tab no longer exists:", chrome.runtime.lastError.message);
        return;
      }
      ...
      if (!shouldIgnoreForIntent(currentUrl) && !promptedTabs.has(tab.id)) {
        console.log(`TabSense : Showing intent popup for new tab ${tab.id} with URL: ${currentUrl}`);
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  console.log(`TabSense : onUpdated - Tab ID: ${tabId}, URL: ${currentUrl}, Status: ${changeInfo.status}`);
  ...
  if (currentUrl && !shouldIgnoreForIntent(currentUrl) && !promptedTabs.has(tabId)) {
    console.log(`TabSense : Tab ${tabId} updated with URL ${currentUrl}, showing intent popup`);
    showIntentPopup(tab);
  }
});
...
function showIntentPopup(tab) {
  ...
  chrome.windows.create(
    ...
    (newWindow) => {
      if (chrome.runtime.lastError) {
        console.error("TabSense : Error creating popup window:", chrome.runtime.lastError.message);
      } else {
        console.log("TabSense : Intent popup window created for tab", tab.id, "Window ID:", newWindow.id);
      }
    }
  );
}
...
function saveTabWithIntent(tabId, url, intent) {
  chrome.storage.local.get(["tabData"], function (result) {
    ...
    chrome.storage.local.set({ tabData: tabData }, function () {
      console.log(`TabSense : Tab ${tabId} saved with intent: ${intent}`);
    });
  });
}
  ...
  if (message.action === "saveIntent") {
    ...
    if (tabId && intent) {
      chrome.tabs.get(tabId, (tabDetails) => {
        if (chrome.runtime.lastError) {
          console.error(
            `TabSense : Error getting tab details for tab ${tabId}:`,
        ...
        if (tabDetails) {
          ...
          if (tabUrl === dashboardUrl) {
            console.log(
              "TabSense : Attempted to save intent for dashboard tab. Ignoring.",
              tabDetails
            );
            sendResponse({
              success: false,
            ...
            if (sender.tab && autoTriggered) {
              chrome.windows.remove(sender.tab.windowId, () => {
                if (chrome.runtime.lastError) console.error("TabSense : Error closing popup for dashboard:", chrome.runtime.lastError.message);
              });
            }
            return;
          }
          ...
          if (sender.tab && autoTriggered) {
            chrome.windows.remove(sender.tab.windowId, () => {
                if (chrome.runtime.lastError) console.error("TabSense : Error closing auto-triggered popup:", chrome.runtime.lastError.message);
                else console.log("TabSense : Auto-triggered popup closed for tab", tabId);
            });
          }
          sendResponse({ success: true });
        } else {
          console.error(`TabSense : Could not find tab with ID ${tabId}`);
          sendResponse({ success: false, error: `Tab ${tabId} not found` });
        }
      });
    } else {
      console.error(
        "TabSense : saveIntent message missing tabId or intent.",
        message
      );
  ...
  if (message.action === "getCurrentTab") {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      if (tabs.length > 0) {
        ...
        if (tabUrl === dashboardUrl) {
          console.log(
            "TabSense : getCurrentTab called, active tab is dashboard."
          );
        ...
        if (tabUrl && isPopupUrl(tabUrl)) {
            console.log("TabSense : getCurrentTab called, active tab is an intent popup itself. Responding with no specific tab.");
        ...
        if (mostRecentTab) {
          console.warn(
            "TabSense : No active tab found for getCurrentTab, falling back to mostRecentTab."
          );
          sendResponse({
            tab: {
              id: mostRecentTab.id,
          ...
          console.error(
            "TabSense : No active tab found and no mostRecentTab for getCurrentTab."
          );
